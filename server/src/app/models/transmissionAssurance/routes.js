...
router.post('/data', validateBody(['timestamp']), gate(12), async ({ body, timestamp }, res) => {
  const result = await processData(timestamp, body.timestamp);
  return res.status(200).return(result);
});

router.post('/big-data', validateBody(['timestamp']), gate(14), async ({ body, timestamp }, res) => {
  const result = await processData(timestamp, body.timestamp);
  return res.status(200).return(result);
});
...
module.exports = Router().use('/transmission-assurance', router);
